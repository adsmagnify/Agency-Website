...
export default function HeroSection() {
  ...
  const renderWord = (text: string) => (
    <span className="inline-block whitespace-nowrap tracking-[0.02em]">
      {text.split("").map((char, cIdx) => (
        <span
          ...
          style={{
            ...
            transition:
              "transform 0.06s ease-out, padding 0.06s ease-out, letter-spacing 0.06s ease-out",
          }}
        >
          {char}
        </span>
  ...
  const renderAccentWord = (text: string) => (
    <span
      className="inline-block relative align-baseline tracking-[0.02em]"
      style={{ transformStyle: "preserve-3d" }}
    >
      <span className="hero-accent-text italic font-normal text-[#004AAD] inline-block relative z-10 drop-shadow-[0_2px_12px_rgba(0,74,173,0.18)]">
        {text.split("").map((char, cIdx) => (
          <span
            key={cIdx}
            className="proxi-char hero-char inline-block will-change-transform"
      ...
      <span className="sparkle-item sparkle-1 pointer-events-none absolute -top-3 -right-2.5 text-[#FFC619] text-base sm:text-xl select-none z-20">
        ✦
      </span>
      ...
      <span className="sparkle-item sparkle-3 pointer-events-none absolute -bottom-1 -right-1 text-[#FFC619] text-xs sm:text-sm select-none z-20">
        ⋆
      </span>

      {/* Decorative Gradient Underline */}
      <span className="magnify-underline pointer-events-none absolute -bottom-0.5 left-0 right-0 h-[2.5px] bg-gradient-to-r from-[#004AAD] via-[#4CA3FF] to-[#FFC619] rounded-full" />
    </span>
  );
  ...
  return (
    ...
    >
      ...
      <div className="relative z-20 w-full px-4 sm:px-6 max-w-4xl mx-auto text-center flex flex-col items-center pt-3 sm:pt-5 md:pt-6 pb-2 sm:pb-3">
        ...
        {/* Interactive Magnifying Glass Lens with 100% Uniform Word Spacing */}
        <Lens lensSize={180} maxStretch={1.38} radius={110} className="w-full">
          <h1
            className="font-serif text-[2.05rem] xs:text-[2.4rem] sm:text-4xl md:text-5xl lg:text-[4.2rem] font-normal leading-[1.16] sm:leading-[1.16] tracking-tight text-[#0A1A3A] max-w-4xl mx-auto py-1 sm:py-2 flex flex-col items-center"
            style={{ perspective: "1000px" }}
          >
            {/* Line 1: 'Marketing that magnifies' (Uniform gap across all words) */}
            <div className="flex flex-wrap sm:flex-nowrap items-baseline justify-center gap-x-2.5 sm:gap-x-3.5 md:gap-x-4">
              <div className="flex items-baseline justify-center gap-x-2.5 sm:gap-x-3.5 md:gap-x-4 whitespace-nowrap">
                {renderWord("Marketing")}
                {renderWord("that")}
              </div>
              <div className="flex items-baseline justify-center">
                {renderAccentWord("magnifies")}
              </div>
            </div>

            {/* Line 2: 'every dollar you spend' (Uniform gap across all words) */}
            <div className="flex flex-wrap sm:flex-nowrap items-baseline justify-center gap-x-2.5 sm:gap-x-3.5 md:gap-x-4 mt-0.5 sm:mt-1">
              {renderWord("every")}
              {renderWord("dollar")}
              {renderWord("you")}
              {renderWord("spend")}
